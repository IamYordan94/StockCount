-- Insert items and shop-item assignments for Damrak 6 shop
-- This migration adds all items from Damrak 6 shop
...
-- Create shop-item assignments for Damrak 6
-- IJSJES (Ice Cream) - Other category items
INSERT INTO shop_items (shop_id, item_id)
SELECT 
  (SELECT id FROM shops WHERE name = 'Damrak 6'),
  id
FROM items
WHERE name IN (
  'OLA Raket',
