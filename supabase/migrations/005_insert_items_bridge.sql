-- Insert items and shop-item assignments for Bridge shop
-- This migration adds all items from Bridge shop
...
-- Create shop-item assignments for Bridge
-- DRANK SHOP items
INSERT INTO shop_items (shop_id, item_id)
SELECT 
  (SELECT id FROM shops WHERE name = 'Bridge'),
  id
FROM items
WHERE name IN (
  'Heineken blik 33CL',
